...
const BalancePayment = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.amount || !form.type) return toast.success("Fill all required fields");
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-red-200 p-4 rounded-md shadow">
          <h3 className="text-lg font-semibold text-gray-700">Total Paid</h3>
          <p className="text-xl font-bold text-red-600">₹ {total("PAID")}</p>
        </div>
        <div className="bg-yellow-200 p-4 rounded-md shadow">
      ...
      <h3 className="text-xl font-semibold mb-2">🔍 All Entries</h3>
      {loading ? (
        <p>Loading...</p>
      ) : (
        <ul className="space-y-2">
          {balances.map((entry) => (
            <li
              ...
              <div>
                <p>
                  <strong>{entry.type}</strong>: <span className="text-[17px]">₹{entry.amount} by{" "}
                  {entry.paymentMethod} </span>
                </p>
                <div className="flex gap-4">
                    {entry.payerName && <p><b>From:</b> {entry.payerName} .</p>}
                    {entry.paidTo && <p><b>To:</b> {entry.paidTo} .</p>}
                </div>
                {entry.description && (
                  <p className="text-[17px] text-gray-600">{entry.description}</p>
                )}
              </div>
              <button
